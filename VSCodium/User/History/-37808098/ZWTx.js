...
const SpeakerBox = (speaker) => Widget.Box({
    vertical: true,
    children: [
        Widget.Label({
            label: speaker.bind('name'),
        }),
        Widget.Label({
            label: speaker.bind('volume').as(v => `Volume: ${Math.floor(v * 100)}%`),
        }),
    ],
});

const DynamicSpeakerList = () => Widget.Box({
    vertical: true,
    children: audio.bind('speakers').as(speakers => 
        speakers.map(speaker => SpeakerBox(speaker))
    ),
});
...
const AudioManager = () => Widget.Window({
	...
	child: Widget.Box({
		...
		children: [
			Widget.Button({
				on_clicked: ()=>{console.log(audio.speakers[0].stream.name); RefreshAudioDevice()}
			}),
