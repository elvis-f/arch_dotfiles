...
let current_audio_device = "none"

const RefreshAudioDevice = () => {
	current_audio_device = audio.speakers[0].stream.name
}
...
const AudioManager = () => Widget.Window({
	...
	child: Widget.Box({
		...
		children: [
			...
			...audio.bind("speakers").map(x => x.stream.name),
			Widget.Label({
				label: audio["speaker"].bind('name')
			}),
			P("Speaker:"),
			Div([
				VolumeIcon(),
